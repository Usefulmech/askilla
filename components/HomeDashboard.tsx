      ...
      >
        <defs>
          ...
          <linearGradient id="penGold" x1="0" y1="1" x2="1" y2="0">
            <stop offset="0%" stopColor="#94401F" />
            <stop offset="50%" stopColor="#E9C496" />
            <stop offset="100%" stopColor="#94401F" />
          </linearGradient>
...
export const HomeDashboard: React.FC = () => {
  ...
  const handleSearch = (topicToSearch: string) => {
    ...
    useAskillaStore.setState({ currentCourse: null, completedModuleIds: [] });
    setScreen("module");
  };

  return (
    <div className="min-h-screen bg-[#F5F5F0] dark:bg-[#121212] text-[#1C1917] dark:text-[#F5F5F4] flex flex-col justify-between pb-24 md:pb-12 md:pl-64 transition-colors duration-200">
      ...
      <header className="w-full px-4 md:px-12 py-4 hidden md:flex items-center justify-between border-b border-[#E0E0E0]/60 dark:border-white/5 sticky top-0 bg-[#F5F5F0]/95 dark:bg-[#121212]/95 backdrop-blur-md z-40 transition-colors">
        <button
          ...
          className="p-2.5 rounded-full bg-white dark:bg-[#1E1E1E] border border-[#E0E0E0] dark:border-white/10 hover:bg-[#F5F5F0] dark:hover:bg-[#121212] text-[#1C1917] dark:text-[#F5F5F4] md:hidden active:scale-95 transition-all"
        >
          <Menu className="w-5 h-5" />
        </button>

        <h1 className="font-heading font-extrabold text-2xl text-[#1C1917] dark:text-[#F5F5F4] tracking-tight md:hidden">
          Askilla
        </h1>

        {/* Desktop Title Header */}
        <div className="hidden md:block text-left">
          <h2 className="font-heading font-extrabold text-lg text-[#1C1917] dark:text-[#F5F5F4]">
            Learning Dashboard
          </h2>
      ...
      <main className="w-full px-4 md:px-12 pt-16 sm:pt-20 md:pt-8 space-y-6 sm:space-y-8 flex-1">
        ...
        >
          <h2 className="font-heading font-extrabold text-3xl sm:text-5xl text-[#1C1917] dark:text-[#F5F5F4] leading-tight min-h-[60px] text-center">
            <span>
              {typedText}
              <span className="inline-block w-1.5 h-7 sm:h-10 ml-1.5 bg-[#C25B32] animate-pulse align-middle rounded-full" />
            </span>
          </h2>
          <p className="text-base sm:text-lg md:text-xl text-[#1C1917]/800 dark:text-[#F5F5F4]/80 font-sans leading-tight font-extrabold max-w-xl mx-auto">
            {language === "pidgin"
              ? "Search any topic. Uncle Sabi go break am down for you."
              : "Search any topic. Uncle Sabi breaks it down for you."}
          </p>
        </motion.div>
        ...
        >
          ...
          >
            <Search className="absolute left-4 sm:left-6 w-5 h-5 sm:w-6 sm:h-6 text-[#1C1917]/55 dark:text-[#F5F5F4]/55 shrink-0" />
            <input
              type="text"
              placeholder="Type a topic, subject, or skill..."
              value={inputTopic}
              onChange={(e) => setInputTopic(e.target.value)}
              className="w-full pl-12 pr-28 sm:pl-16 sm:pr-40 py-4 sm:py-5 bg-white dark:bg-[#1E1E1E] border-2 border-[#E0E0E0] dark:border-white/10 text-[#1C1917] dark:text-[#F5F5F4] placeholder-[#1C1917]/40 dark:placeholder-[#F5F5F4]/40 focus:outline-none focus:border-[#C25B32] text-sm sm:text-lg rounded-full shadow-sm font-sans transition-all duration-200"
            />
            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center">
              <button
                type="submit"
                disabled={isLoadingModule || !inputTopic.trim()}
                className="px-5 py-2.5 sm:px-8 sm:py-3.5 bg-[#C25B32] text-[#1C1917] text-xs sm:text-base font-extrabold rounded-full hover:bg-[#94401F] active:scale-95 disabled:opacity-50 transition-all flex items-center gap-2 shadow-sm animate-breathing"
              >
        ...
        <motion.div
          className="bg-white dark:bg-[#1E1E1E] rounded-3xl p-4 sm:p-6 border border-[#E0E0E0] dark:border-white/10 shadow-sm space-y-4 text-left w-full overflow-hidden"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, delay: 0.2 }}
        >
          {/* Header & Category Tabs */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E0E0E0]/60 dark:border-white/5 pb-3">
            <div className="flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-[#C25B32] shrink-0" />
              <h2 className="text-xs sm:text-sm font-extrabold text-[#1C1917] dark:text-[#F5F5F4] uppercase tracking-wider">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-[11px] sm:text-xs font-bold shrink-0 whitespace-nowrap transition-all flex items-center gap-1.5 ${
                      active
                        ...
                        : "bg-[#F5F5F0] dark:bg-[#2C2C2C] text-[#1C1917]/70 dark:text-[#F5F5F4]/70 hover:bg-[#FDEEE9]/60"
                    }`}
                  >
          ...
          <div className="relative group">
            ...
            <button
              ...
              className="hidden md:flex absolute -left-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white dark:bg-[#1E1E1E] border-2 border-[#C25B32] shadow-xl items-center justify-center text-[#1C1917] dark:text-[#F5F5F4] hover:bg-[#FDEEE9] dark:hover:bg-[#2D1F1A] hover:scale-110 active:scale-95 transition-all opacity-0 group-hover:opacity-100"
            >
              <ChevronLeft className="w-6 h-6 text-[#C25B32]" />
            </button>
            ...
            >
              {(trendingItems.length > 0 ? trendingItems : trendingTopics.map((t, i) => ({
                ...
                <button
                  ...
                  className="snap-center sm:snap-start shrink-0 w-[88vw] max-w-[340px] sm:w-[350px] md:w-[380px] p-5 sm:p-7 rounded-3xl bg-[#FDEEE9]/45 dark:bg-[#2D1F1A]/25 border-2 border-[#C25B32]/40 hover:border-[#C25B32] text-left transition-all active:scale-[0.98] flex flex-col justify-between space-y-3.5 group hover:shadow-xl shadow-sm"
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <span className="px-2.5 py-0.5 bg-[#C25B32]/20 border border-[#C25B32]/40 text-[#C25B32] text-[9px] sm:text-[10px] font-extrabold rounded-full uppercase tracking-wider truncate max-w-[65%]">
                        {item.domainLabel}
                      </span>
                      <span className="text-[9px] sm:text-[10px] text-[#1C1917]/60 dark:text-[#F5F5F4]/60 font-extrabold uppercase truncate">
                        {item.tag}
                      </span>
                    </div>

                    <h3 className="font-heading font-extrabold text-sm sm:text-lg text-[#1C1917] dark:text-[#F5F5F4] group-hover:text-[#C25B32] transition-colors line-clamp-1">
                      {item.topic}
                    </h3>

                    <p className="text-xs sm:text-sm text-[#1C1917]/75 dark:text-[#F5F5F4]/75 font-sans font-medium line-clamp-2 leading-relaxed min-h-[2.5rem]">
                      {item.description}
                    </p>
                  </div>

                  <div className="flex items-center justify-between pt-2.5 border-t border-[#C25B32]/25 text-[11px] sm:text-xs font-bold text-[#1C1917]/70 dark:text-[#F5F5F4]/70">
                    <span className="truncate">{item.learnerCount}</span>
                    <div className="flex items-center gap-1 text-[#C25B32] font-extrabold shrink-0 group-hover:translate-x-1 transition-transform">
            ...
            <button
              ...
              className="hidden md:flex absolute -right-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white dark:bg-[#1E1E1E] border-2 border-[#C25B32] shadow-xl items-center justify-center text-[#1C1917] dark:text-[#F5F5F4] hover:bg-[#FDEEE9] dark:hover:bg-[#2D1F1A] hover:scale-110 active:scale-95 transition-all opacity-0 group-hover:opacity-100"
            >
              <ChevronRight className="w-6 h-6 text-[#C25B32]" />
            </button>
          </div>
        </motion.div>
        ...
        {/* Recent / Continue Learning */}
          <motion.div
            className="bg-white dark:bg-[#1E1E1E] rounded-3xl p-6 border border-[#E0E0E0] dark:border-white/10 shadow-sm space-y-4 text-left"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.4, delay: 0.3 }}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm font-extrabold text-[#1C1917] dark:text-[#F5F5F4] uppercase tracking-wider">
                <BookOpen className="w-5 h-5 text-[#C25B32]" />
                <span>Active Track</span>
              </div>

              {currentCourse && completedModuleIds.length >= currentCourse.modules.length && (
                <span className="px-2.5 py-1 bg-[#C25B32]/20 border border-[#C25B32]/40 text-[#C25B32] text-[10px] font-extrabold rounded-full uppercase">
                  100% Completed
                </span>
              )}
            </div>
            ...
            {currentCourse ? (
              <button
                ...
                className="w-full bg-[#FDEEE9]/50 dark:bg-[#1E1E1E]/60 border border-[#C25B32]/40 hover:border-[#C25B32] rounded-3xl p-5 sm:p-6 text-left flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:shadow-md group active:scale-[0.98]"
              >
                <div className="flex-1 min-w-0 w-full">
                  <div className="flex items-center gap-2 mb-1.5">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#C25B32] animate-pulse" />
                  ...
                  <h3 className="font-heading font-bold text-base text-[#1C1917] dark:text-[#F5F5F4] truncate">
                    {currentCourse.topic}
                  </h3>
                  <p className="text-xs sm:text-sm text-[#1C1917]/70 dark:text-[#F5F5F4]/70 font-sans font-medium mt-1 line-clamp-2">
                    {completedModuleIds.length >= currentCourse.modules.length
                  ...
                  <div className="w-full h-2.5 bg-[#E0E0E0] dark:bg-white/10 rounded-full overflow-hidden mt-3 shadow-inner">
                    <div 
                ...
                <div className="flex items-center justify-end sm:justify-start gap-2 text-[11px] sm:text-sm font-extrabold text-[#C25B32] transition-all sm:group-hover:translate-x-1 shrink-0 w-full sm:w-auto uppercase sm:capitalize pt-2 sm:pt-0 border-t sm:border-t-0 border-[#C25B32]/10 sm:border-transparent mt-2 sm:mt-0">
                  <span>
                    {completedModuleIds.length >= currentCourse.modules.length ? "View Win Card" : "Continue"}
                  </span>
                  <div className="flex items-center justify-center w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-[#C25B32] text-[#1C1917] dark:text-[#1E1E1E] shadow-sm shrink-0">
                    <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  </div>
                </div>
              </button>
            ) : (
              <div className="p-5 rounded-2xl bg-[#F5F5F0]/65 dark:bg-[#1E1E1E]/40 border border-dashed border-[#E0E0E0] dark:border-white/10 text-center space-y-2 py-8">
                <p className="text-xs sm:text-sm font-sans font-semibold text-[#1C1917]/55 dark:text-[#F5F5F4]/55">
                  No active tracks yet.
                </p>
                <p className="text-[11px] font-sans text-[#1C1917]/45 dark:text-[#F5F5F4]/45">
      ...
      {/* Footer */}
      <footer className="w-full px-4 md:px-12 py-4 text-center text-[11px] text-[#1C1917]/40 dark:text-[#F5F5F4]/40 border-t border-[#E0E0E0]/40 dark:border-white/10/40 mt-12">
        ...
        {isLoadingModule && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-white dark:bg-[#1E1E1E] rounded-3xl p-8 max-w-sm w-full border-2 border-[#C25B32] dark:border-[#94401F] shadow-2xl space-y-6"
            >
              {/* Learning / Course Building Animation */}
              <div className="relative w-28 h-28 mx-auto flex items-center justify-center">
                <div className="absolute inset-0 rounded-full border-4 border-dashed border-[#C25B32]/40 animate-spin" style={{ animationDuration: '8s' }} />
                <div className="absolute inset-2 rounded-full border-4 border-[#C25B32]/10 border-t-[#C25B32] animate-spin" style={{ animationDuration: '1.5s' }} />
                
                <div className="relative flex items-center justify-center z-10">
                  <BookOpen className="w-10 h-10 text-[#C25B32] animate-pulse" />
                  <Sparkles className="w-4 h-4 text-[#C25B32] absolute -top-3 -right-2 animate-bounce" />
                </div>
              </div>
              
              <div className="space-y-2">
                <h3 className="font-heading font-extrabold text-xl text-[#1C1917] dark:text-[#F5F5F4] flex items-center justify-center gap-1.5">
                  <span>Uncle Sabi is generating...</span>
                </h3>
                <p className="text-sm text-[#1C1917]/70 dark:text-[#F5F5F4]/70 font-sans font-semibold">
                  Compiling your custom learning modules. Sabi power takes 15 seconds!
                </p>
              </div>

              {/* Progress bar animation */}
              <div className="w-full h-2 bg-[#E0E0E0] dark:bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-[#C25B32] rounded-full animate-pulse" style={{ width: '80%' }} />
              </div>
            </motion.div>
          </motion.div>
        )}
